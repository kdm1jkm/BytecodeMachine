...
Compiler::Compiler(const std::string &filename)
        : in(filename, std::ios::binary),
          output((filename.find_last_of(".") == std::string::npos) ? filename : filename.substr(0,
                                                                                                (filename.find_last_of(
                                                                                                        ".")))) {
}

Compiler::Compiler(const std::string &filename, const std::string &output) : in(filename, std::ios::binary),
                                                                             output(output) {
}
...
void Compiler::compile() {
    ...
    while (!in.eof()) {
        ...
        if (convertUpper(word) == "LABEL") {
            ...
            if (SymbolTable.find(convertUpper(label)) != SymbolTable.end()) {
                // 라벨로 쓸 수 없음
                printError([&label](int line, int column) {
                    std::cout << "\"" << label << "\" cannot be used as label. [" << line << ":" << column << "]"
                              << std::endl;
                });
                return;
            }
            ...
            if (symbolIter->second == Symbol::POP || symbolIter->second == Symbol::PUSH) {
                ...
                if (literal.find_first_of("0123456789") == std::string::npos) {
                    printError([&literal](int line, int column) {
                        std::cout << "\"" << literal << "\" is not literal.[" << line << ":" << column << "]"
                                  << std::endl;
                    });
                    return;
                }
    ...
    std::for_each(commands.begin(), commands.end(), [&out](uint8_t command) { out.write((char *) &command, 1); });
    out.close();
}
